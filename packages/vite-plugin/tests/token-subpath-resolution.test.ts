...
`
  ...
  it('works in a Node consumer app through the JS token subpath export', async () => {
    ...
    const tempPackageDist = path.join(packageDir, 'dist');
    const coreDist = path.join(corePackageDir, 'dist');
    ...
    await fs.mkdir(coreDist, { recursive: true });
    ...
    await fs.writeFile(
      path.join(coreDist, 'token.js'),
      `export function token() { return undefined; }
export function tokenValue(path, options = {}) {
  const values = options.values ?? undefined;
  if (!values || !Object.hasOwn(values, path)) return undefined;
  return values[path];
}
`
    );
